...
import { updateProfile, updatePassword } from "@/app/(app)/settings/action"

export default function SettingsForm({ initialUser, initialRole }) {
  const [state, formAction, isPending] = useActionState(updateProfile, null)
  const [passwordState, passwordFormAction, isPasswordPending] = useActionState(updatePassword, null)
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-2 flex flex-col gap-8">
        ...
        <div className="bg-surface border border-border/40 rounded-2xl p-6 shadow-sm transition-all duration-300 hover:shadow-md">
        ...
        <div className="bg-surface border border-border/40 rounded-2xl p-6 shadow-sm transition-all duration-300 hover:shadow-md">
          <h2 className="font-sans text-xl font-bold text-navy mb-1">Change Password</h2>
          <p className="text-xs text-zinc-500 mb-6">Update your account password. Use a strong, unique password.</p>

          <form action={passwordFormAction} className="space-y-4">
            <div>
              <label htmlFor="current_password" className="mb-1 block text-xs font-semibold text-navy">
                Current Password
              </label>
              <input
                type="password"
            ...
            <div>
              <label htmlFor="password" className="mb-1 block text-xs font-semibold text-navy">
                New Password
              </label>
              <input
                type="password"
            ...
            {passwordState?.error && (
              <div className="p-3 bg-danger/5 border border-danger/20 rounded-lg flex items-center gap-2">
                ...
                <p className="text-xs font-medium text-danger">{passwordState.error}</p>
              </div>
            )}
            ...
            {passwordState?.success && (
              <div className="p-3 bg-teal/5 border border-teal/20 rounded-lg flex items-center gap-2">
                ...
                <p className="text-xs font-medium text-teal">{passwordState.success}</p>
              </div>
            )}
